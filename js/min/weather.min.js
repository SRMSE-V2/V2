...
WEATHER.find(".weather_city").text(city.toLowerCase().capitalizeMe()),WEATHER.find(".weather_city").append("<br/>"),WEATHER.find(".weather_city").append('<span style="font-weight:400;line-height:1.3em;max-width:200px;display:block;">'+window.SA.forecast.capitalizeMe()+"</span>")
...
var row=$("<div></div>")
row.addClass("row")}++i}WEATHER.find(".days_weather").append(row),WEATHER.find(".weather_max").html(maxx+"/"+minn+"<sup>&deg;C</sup>"),WEATHER.find(".weather_moonrise").text(moonrise),WEATHER.find(".weather_moonset").text(moonset),WEATHER.find(".weather_today_sunset").text(today_sunset),WEATHER.find(".weather_tomorrow_sunrise").text(tomorow_sunrise),$("head").append("<style>#weather{font-size:18px !important;text-align: left;}.weather_city{font-size:36px;}.weather_city>span{font-size:18px;}</style>"),$("#smart_col").html(WEATHER),$("#smart_col").removeClass("hide")}