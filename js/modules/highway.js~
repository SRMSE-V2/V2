if($("#highway").length===0){
var highway=$("<div id=\"highway\" class=\"module col-md-12 well alpha-blur\" style=\"border:0px;margin-top:20px;color:white;\"> <div class=\"row\" style=\"text-align:center\"> <img src=\"/main/images/highway.png\" height=\"62\" width=\"65\"> </div> <div class=\"row\" style=\"text-align:center;padding-top:5px\"> <h4 id=\"highway_highway\"></h4> <h5><span id=\"route\"></span></h5><button style=\"height:18px;padding:0px;font-size:12px;\" class=\"btn btn-primary\">More</button> <h6 id=\"highway_length_state\"></h6> <h5><b>Total Length - </b><span id=\"highway_length_total\"></span></h5> </div><div class=\"row\" style=\"text-align:center;\">                     <div style=\"float:left;width:50&percnt;;height:34px;\">                        <button id=\"high_prev\" style=\"background-color:#008EFF;\" class=\"btn btn-primary glyphicon glyphicon-chevron-left\" value=\"Previous\"/>                     </div>                     <div style=\"float:left;width:50&percnt;;height:34px;\">                     <button id=\"high_next\" style=\"background-color:#008EFF;\" class=\"btn btn-primary glyphicon glyphicon-chevron-right\" value=\"Next\"/>                     </div>                  </div>          </div>");
...
$("#route").text(window.val["ans"][0]["route"].slice(0,3).join(" "));
...
$("#high_prev").on("click", function() {
   ...
   if (window.high.length > 1 && window.chigh < window.high.length && window.chigh !== 0) {
	...
	$("#route").text(val["route"].slice(0,3).join(" "));
