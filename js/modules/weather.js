...
WEATHER.find(".img_today").css("-moz-filter");
				...
				WEATHER.find(".weather_desc").append('<div>'+window.SA["forecast"].capitalizeMe()+'</div>');
                                var w = [];
                                var i = 0;
                                var row = $("<div></div>");
                                row.addClass("row");
...
$('head').append("<style>.weather_city{font-size:36px;}.weather_city>span{font-size:18px;}.weather_max{text-align:center;}.weather_desc{font-size:16px;}</style>");
